...
if (isset ($_POST["id"]))
{
	$query = "select * from tbl_user_details where user_id=".$_POST["id"];
	...
	jQuery(document).ready(function(){
		jQuery("#editcategory").submit(function(event){
			...
			jQuery.post("edit_user.php",jQuery(this).serialize(),function(data,status){
				
				if(data.trim()=="success")
				{
					window.location.href="user_listing.php?msg=User Updated Successfully";
				}
				else
				{
					window.location.href="user_listing.php?msg= Failed to Update User ";
...
else if (isset ($_POST["name"]))
{
	 $query = "update  tbl_user_details set `first_name`='".$_POST["name"]."' , `last_name`='".$_POST["lastName"]."' ,`email`='".$_POST["email"]."' ,`mobile`='".$_POST["mobile"]."' ,`status`=".$_POST["status"]." where user_id=".$_SESSION["user_edit_id"];
			 ...
			 if($_POST["password"]!="")
			 {
			$query = "update  tbl_user set `password`='".$_POST["password"]."'  where id=".$_SESSION["user_edit_id"];
