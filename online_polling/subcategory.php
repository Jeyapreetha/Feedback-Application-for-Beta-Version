...
<?php
	 $query = "select * from tbl_sub_category where category_id = ".$_POST["cat_id"];
...
jQuery(document).ready(function(){
	...
	jQuery.post("edit_subcategory.php",{id:jQuery(this).attr("data-id")},function(data,status){
...
jQuery("#btnAdd").click(function(){
	jQuery.post("add_subcategory.php",function(data,status){
...
function delete_category(){
	jQuery.post("delete_subcategory.php",{id:id},function(data,status){
		if(data.trim()=="success")
		{
			window.location.href="subcategory.php?msg=Category Deleted Successfully";
			
		}
		else{
			window.location.href="subcategory.php?msg=Failed to  Delete Category";
...
?>
	  ...
	  <h1 class="pagehead">Sub-Category
	  <button id="btnAdd" type="button" class="btn btn-info" >Add Category</button>
</h1>
    
<table border=1 class="table table-striped">
<tr class="head">
					<th><label for="inputUser" class=" control-label">Category Name</label></th>
					<th><label for="inputUser" class=" control-label">Sub-Category</label></th>
...
<?php
		  ...
		  $query = "select a.id,a.sub_name,a.category_id,a.status,b.name 
		 from tbl_sub_category a LEFT JOIN tbl_category b ON a.category_id = b.id";
			 $result= $conn->query($query);
			 if($result->num_rows!=0)
			 {
				 $statusName="";
